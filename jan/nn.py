...
class NN:
    ...
    def __init__(self, dims, activations, loss, optimizer):
        ...
        for i in range(len(dims) - 1):
            self.w[i + 1] = np.random.randn(dims[i], dims[i + 1]) / np.sqrt(dims[i])  # Xavier init
    ...
    def _backward(self, z, a, y):
        ...
        Parameters:
            z: The output of each layer.
            a: The output of each activation function.
            y: Ground truths.
        ...
        for i, (dw, delta) in update_params.items():
            self.optimizer.step(weights=self.w[i], gradients=dw)
            self.optimizer.step(weights=self.b[i], gradients=np.mean(delta, axis=0))
    ...
    def partial_fit(self, X, y):
        ...
        Parameters:
            X: An array of shape (n_samples, n_features).
            y: An array of shape (n_samples, n_targets). This is expected to be a 2D array, even
